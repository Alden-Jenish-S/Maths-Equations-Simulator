...
- `CURVES`, `CURVE_LIST`: 17 entries: circle, ellipse, hyperbola, parabola, involute, superellipse, lemniscate, cardioid, deltoid, nephroid, archimedean, logarithmic, clothoid, helix, lissajous, torus-knot, and joukowsky. Each `{id,name,family,equation,description,defaults,parameters,domain:[min,max],closed}`. Every schema `{label,min,max,step,default}`; integer winding parameters additionally expose `integer: true`.
